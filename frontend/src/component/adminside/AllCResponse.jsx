...
function AllCResponse() {
  ...
  useEffect(() => {
    const getContacts = async () => {
      try {
        const response = await axios.get("http://localhost:5000/contactus/read");
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDeletion) {
      axios
        .delete(`http://localhost:5000/contactus/delete/${id}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  const handleSendMail = async (email, name) => {
    try {
      await axios.post("http://localhost:5000/email/send-mail", {
        name: name,
        email: email,
      });
